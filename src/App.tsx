...
// =================================================================
// 【重要】ここに先ほどFirebaseの画面で見つけたコードを貼り付けてください！
// const firebaseConfig = { ... } の部分を上書きします。
// =================================================================
...
function Room({ user, roomCode, role, nickname, onLogout, db, appId }) {
  ...
  const handleNextQuestion = async () => {
    ...
    const historyEntry = {
      ...
      timestamp: new Date().toISOString(),
      correctAnswer: roomData.correctAnswer || null, // 正解があれば保存、なければnull
    };
  ...
  const downloadCSV = () => {
    ...
    // 履歴データ（各履歴のcorrectAnswerを使用）
    if (roomData.history && Array.isArray(roomData.history)) {
      roomData.history.forEach((entry) =>
        addRows(entry.responses, entry.question, entry.correctAnswer)
      );
    }
    // 現在の問題データ
    ...
    // 【修正】Blobを使ってより確実に保存する（ハングル等の文字化け防止強化）
